...
public class NativeHook implements NativeKeyListener, NativeMouseInputListener, NativeMouseWheelListener {
  ...
  static NativeHookLog log = new NativeHookLog();
...
//  }
  ...
  private void init() {
    if (eventQueueSize == 0) {
      mainMonitor = null; //SX.getSXLOCALDEVICE().getMonitor(SX.getSXLOCALDEVICE().getMonitorID());
  ...
  public List<Object> getQueue() {
    ...
    if (null != evt) {
      while (null != (evt = eventQueue.poll())) {
        content.add(evt);
      }
    }
